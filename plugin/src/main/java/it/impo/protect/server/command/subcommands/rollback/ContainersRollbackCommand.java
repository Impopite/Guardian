...
public class ContainersRollbackCommand {

    private final Protect plugin;

    public ContainersRollbackCommand(Protect plugin) {
        this.plugin = plugin;
    }

    public dev.jorel.commandapi.CommandAPICommand get() {
        LangLoader lang = plugin.getLangLoader();
        return new dev.jorel.commandapi.CommandAPICommand("containers")
                ...
                .executes((sender, args) -> {
                    ...
                    plugin.getRollbackManager().rollbackContainers(player, (int) args.get("raggio"), java.time.Duration.ofSeconds(seconds));
                });
    }
}
