...
import it.impo.protect.api.data.RollbackSummary;
...
public class BaseRollbackManager extends RollbackManager {
    ...
    public BaseRollbackManager(Protect plugin) {
        this.plugin = plugin;
    }
    ...
    public void rollbackBlocks(Player player, int radius, Duration duration) {
        ...
        plugin.getProtectTable().getBlockLogTable().rollbackLogs(center, radius, since).thenAccept(logs ->
    ...
    public void rollbackContainers(Player player, int radius, Duration duration) {
        ...
        plugin.getProtectTable().getContainerLogTable().rollbackLogs(center, radius, since).thenAccept(logs ->
                Bukkit.getScheduler().runTask(plugin, () -> {
