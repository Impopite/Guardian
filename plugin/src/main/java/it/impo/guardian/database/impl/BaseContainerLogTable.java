...
public class BaseContainerLogTable extends ContainerLogTable {
    ...
    private static final Logger LOGGER = Logger.getLogger(BaseContainerLogTable.class.getName());
    ...
    private static final String ADD_CONTAINER_LOG = """
        INSERT INTO guardian_container_log (user_uuid, username, world, x, y, z, container_type, item, amount, action, staff)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """;

    @Language("SQL")
    private static final String REMOVE_CONTAINER_LOG = "DELETE FROM guardian_container_log WHERE id = ?";

    @Language("SQL")
    private static final String REMOVE_OLD_CONTAINER_LOG = "DELETE FROM guardian_container_log WHERE date < (NOW() - INTERVAL ? DAY)";

    @Language("SQL")
    private static final String COUNT_CONTAINER_LOG = "SELECT COUNT(*) FROM guardian_container_log WHERE world = ? AND x = ? AND y = ? AND z = ?";
    ...
    @Language("SQL")
    private static final String ROLLBACK_CONTAINER_LOGS = """
        SELECT id, user_uuid, username, world, x, y, z, container_type, item, amount, action, staff, date
        FROM guardian_container_log
        WHERE world = ?
          AND date >= ?
    ...
    @Language("SQL")
    private static final String COUNT_CONTAINER_LOG_BY_PLAYER = "SELECT COUNT(*) FROM guardian_container_log WHERE username = ?";
    ...
    public CompletableFuture<Boolean> addLog(ContainerLog log) {
        return supplyAsync(() -> {
            try (Connection c = dataSource.getConnection();
                ...
                LOGGER.log(Level.SEVERE, "[Guardian] Database error", e);
                return false;
            }
        });
    }
    ...
    @Override
    public CompletableFuture<List<ContainerLog>> rollbackLogs(BasicLocation center, int radius, LocalDateTime since) {
        return supplyAsync(() -> {
            List<ContainerLog> logs = new ArrayList<>();
            try (Connection c = dataSource.getConnection();
                 PreparedStatement ps = c.prepareStatement(ROLLBACK_CONTAINER_LOGS)) {
